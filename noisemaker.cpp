...
void NoiseMaker::generateNoisyImage(std::vector<RGBA>& animationFrame, int frameWidth, int frameHeight,
    ...
    // (2) create blurred animationFrame (only need coarsest level) (done in processImagePyramids)
    // (3) create image pyramids with gaussian blur of textureImage (done in processImagePyramids)
    // (4) loop through image pyrmaid array starting with most blurred
...
void NoiseMaker::createImagePyramids(
    ...
    texturePyramid.resize(PYRAMID_LEVELS);
    texturePyramidDims.resize(PYRAMID_LEVELS);
