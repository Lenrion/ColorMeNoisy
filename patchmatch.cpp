...
float PatchMatch::DistPatch(const Mat & PatchA, const Mat & PatchB, int PatchSize)
{
    return PatchSize * PatchSize * norm(PatchA, PatchB) ;
}


void PatchMatch::GuessAndImprove(const Mat & SourceImage, const Mat & TargetImage, const Mat & mask,
...
void PatchMatch::PatchMatch(const Mat & SourceImage,const Mat & TargetImage, const Mat & Mask,  int nPatchSize, Mat & NearestNeighbor)
{
    // 最近邻数据
